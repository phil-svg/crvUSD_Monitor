...
async function getLiquidatedUsers(controllerAddress: string, startBlock: number, endBlock: number) {
  const controllerContract = new web3HttpProvider.eth.Contract(ABI_Controller, controllerAddress);

  let users: { address: string; blockNumber: number }[] = [];

  const events: any = await getPastEvents(controllerContract, 'Liquidate', startBlock, endBlock);
...
async function getPositionEffectingEvents(controllerAddress: string, startBlock: number, endBlock: number) {
  const controllerContract = new web3HttpProvider.eth.Contract(ABI_Controller, controllerAddress);
  ...
  const removeCollateralEvents = await getPastEventsExtended(
    controllerContract,
    'RemoveCollateral',
    startBlock,
    endBlock
...
async function getLastInteractedBlockData(
  liquidatedUsers: {
    address: string;
    blockNumber: number;
  }[],
  positionEffectingEvents: {
    repayEvents: any[];
    borrowEvents: any[];
    removeCollateralEvents: any[];
  }
) {
  const result = [];
...
async function handleMarket(market: any, startBlock: number, endBlock: number) {
  ...
  const results: { health: number; blocksTillDeath: number; numBands: number }[] = [];
  ...
  for (let userIndex = 0; userIndex < lastInteractedBlockJSON.length; userIndex++) {
    ...
    if (lastInteractedBlock === 0) continue;
    ...
    for (let i = 0; i < numDays * 1; i++) {
      ...
      console.log(
        `User ${userIndex + 1}/${lastInteractedBlockJSON.length}:`,
        numDays,
        health,
        blocksTillDeath,
        numBands
...
async function formatJSON() {
  ...
  const bands4Data = data.filter(
    (item: { health: number; numBands: number }) => item.health > 0 && item.numBands === 4
  );
  const bands10Data = data.filter(
    (item: { health: number; numBands: number }) => item.health > 0 && item.numBands === 10
  );

  // Process data for 4 bands
  const healthByBlock4 = new Map<number, number[]>();
  bands4Data.forEach((entry: { health: number; blocksTillDeath: number }) => {
    ...
    healthByBlock4.get(dayKey)?.push(entry.health);
  });

  // Process data for 10 bands
  const healthByBlock10 = new Map<number, number[]>();
  bands10Data.forEach((entry: { health: number; blocksTillDeath: number }) => {
    const daysTillDeath = Number(((entry.blocksTillDeath * 12) / (60 * 60 * 24)).toFixed(2)); // Convert to days with 2 decimal places
    // Skip if more than 150 days
    if (daysTillDeath > 150) return;
    const dayKey = Math.floor(daysTillDeath * 100) / 100; // Round to 2 decimal places
    ...
    healthByBlock10.get(dayKey)?.push(entry.health);
  });
...
async function getConversionZonePriceRange(controllerAddress: string, user: string, blockNumber: number) {
...
async function getIfInSL(
  ammAddress: string,
  conversionZonePriceRange: string[],
  blockNumber: number
): Promise<boolean> {
  ...
  const isInSl =
    Number(priceOracle) >= Number(conversionZonePriceRange[1]) &&
...
async function handleMarketBands(market: any, startBlock: number, endBlock: number) {
  const controllerAddress = market.returnValues.controller;
  const ammAddress = market.returnValues.amm;
  ...
  const bandCounts: { [key: number]: number } = {};
