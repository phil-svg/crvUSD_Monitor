...
async function getUsersDebtInMarket(user: string, market: MarketData, blockNumber: number) {
  const abi = getMintMarketControllerAbi();
...
async function handleMarket(market: MarketData, blockNumber: number, userDebtMap: Map<string, number>) {
...
export async function checkWhales() {
  ...
  const userDebtMap = new Map<string, number>();
