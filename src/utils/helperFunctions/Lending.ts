...
export async function considerOnlyBoost(actorAddress: string, txHash: string): Promise<string> {
  try {
    const receipt = await getTxReceiptClassic(txHash);
    const needle = ONLY_BOOST_ADDRESS.slice(2).toLowerCase();
    if (receipt && JSON.stringify(receipt.logs).toLowerCase().includes(needle)) return ONLY_BOOST_ADDRESS;
  } catch {
