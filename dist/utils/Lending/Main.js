...
async function processSingleControllerEvent(controllerContract, lendingMarketAddress, event, eventEmitter) {
    ...
    if (event.event === "Borrow") {
        ...
        const message = buildLendingMarketBorrowMessage(txHash, agentAddress, parsedBorrowedAmount, parsedCollatAmount, collatName, collatAddress, positionHealth, lendingMarketAddress, totalDebtInMarket, collatDollarAmount, dollarAmountBorrow);
    ...
    if (event.event === "Repay") {
        ...
        const message = buildLendingMarketRepayMessage(txHash, lendingMarketAddress, positionHealth, totalDebtInMarket, agentAddress, parsedRepayAmount, collatName, collatAddress, collatDollarAmount, parsedCollatAmount, repayDollarAmount);
        console.log("Sending Message (Via processSingleControllerEvent)");
        eventEmitter.emit("newMessage", message);
    }
    if (event.event === "RemoveCollateral") {
        ...
        const collatName = await getCoinSymbol(collatAddress, web3);
        const totalDebtInMarket = await getTotalDebtInMarket(controllerContract, event.blockNumber);
        const message = buildLendingMarketRemoveCollateralMessage(parsedCollatRemovalAmount, txHash, lendingMarketAddress, agentAddress, positionHealth, collatDollarAmount, collatAddress, collatName, totalDebtInMarket);
        console.log("Sending Message (Via processSingleControllerEvent)");
        eventEmitter.emit("newMessage", message);
    }
    // HARD-LIQUIDATION
...
async function processSingleAmmEvent(controllerContract, lendingMarketAddress, event, eventEmitter) {
    if (event.event === "TokenExchange") {
        ...
        const message = buildSoftLiquidateMessage(txHash, lendingMarketAddress, agentAddress, parsedSoftLiquidatedAmount, collatAddress, collatName, collatDollarAmount, parsedRepaidAmount, repaidCrvUSDDollarAmount);
