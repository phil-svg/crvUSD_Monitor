...
export async function decode1Inch(tx) {
    ...
    const methodAbi = abiSwap1Inch.find((abiItem) => web3.eth.abi.encodeFunctionSignature(abiItem) === methodSignature);
    if (methodAbi) {
        // decode parameters
...
export async function decode1InchV5(tx) {
    ...
    console.log("tx", tx);
    // find corresponding method in abi
    const methodAbi = abi1inchV5.find((abiItem) => web3.eth.abi.encodeFunctionSignature(abiItem) === methodSignature);
    if (methodAbi) {
        // decode parameters
        const decodedParams = web3.eth.abi.decodeParameters(methodAbi.inputs, "0x" + tx.input.slice(10));
        // map decoded parameters to property names
