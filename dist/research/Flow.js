...
async function handleMarket(market, startBlock, endBlock, fleshlending_events) {
    ...
    for (const CONTROLLER_EVENT of PAST_EVENTS_crvUSD_CONTROLLER) {
        if (CONTROLLER_EVENT.event === 'Borrow') {
            const formattedEventData = await handleBorrowEvent(CONTROLLER_EVENT, controllerAddress, collateralAddress, AMM_ADDRESS, fleshlending_events, PAST_EVENTS_crvUSD_CONTROLLER);
            const user = CONTROLLER_EVENT.returnValues.user;
...
async function checkCollateralAddedWithoutRepay(event, PAST_EVENTS_crvUSD_CONTROLLER) {
    const blockNumber = event.blockNumber;
    const buyer = event.returnValues.user;
    // Filter events for the same buyer
    const buyerEvents = PAST_EVENTS_crvUSD_CONTROLLER.filter((controllerEvent) => { var _a; return ((_a = controllerEvent.returnValues) === null || _a === void 0 ? void 0 : _a.user) === buyer; });
    // Start checking from the given block number
    for (const currentEvent of buyerEvents) {
        if (currentEvent.blockNumber <= blockNumber)
            continue;
        if (currentEvent.event === 'Borrow' &&
            ...
            const hasRepayInBetween = buyerEvents.some((intermediateEvent) => intermediateEvent.event === 'Repay' &&
...
async function handleBorrowEvent(event, controllerAddress, collateralAddress, AMM_ADDRESS, fleshlending_events, PAST_EVENTS_crvUSD_CONTROLLER) {
    ...
    if (manualLev)
        mintedViaManualLev += loan_increase;
    return {
        blockNumber,
        loan_increase,
        collateralName,
...
export async function checkTokenFlow() {
    ...
    const FLASHLENDING_EVENTS = await getPastEvents(FLASHLENDER_CONTRACT, 'FlashLoan', crvUSD_LAUNCH_BLOCK, currentBlockNumber);
    const ADDED_MARKET_EVENTS = await getPastEvents(crvUSD_ControllerFactory, 'AddMarket', crvUSD_LAUNCH_BLOCK, currentBlockNumber);
